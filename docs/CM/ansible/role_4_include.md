...
**此处第2步，不要直接将`/root/.ssh`目录删除，删除后会Ansible到被控主机的免密登陆失败，下次执行Ansible命令就会报错了。**
...
在`roles/base/files`目录配置一个`alias.sh`的别名文件：
...
更新`roles/base/tasks/main.yml`文件：
...
你如果你别的别名要配置，直接在`alias.sh`后面追加就好了。
